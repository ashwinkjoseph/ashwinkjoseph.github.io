...
<div style="width:100%; height:55px;" class="wrapperr">
  ...
  <form class="login" method="POST">
    <p class="title">Add Theatres</p>
    <input type="text" placeholder="Place" name="place" autofocus/>
    <i class="fa fa-user"></i>
    <input type="text" name="Theatre" placeholder="theatrename" />
    <i class="fa fa-key"></i>
    <button name="submit">
      <i class="spinner"></i>
...
<?php
    if(isset($_POST["submit"])){
        ...
        $COMMAND="SELECT * FROM theatres";
        $S=mysqli_query($conn, $COMMAND);
        $flag=FALSE;
        while($temp=mysqli_fetch_array($S)){
            if(($temp['Theatre_Name']==$theatrename)&&($temp['Place']==$place)){
                $flag=TRUE;
            }
        }
        if(!$flag){
            $command="INSERT INTO theatres values(id,'$place','$theatrename')";
            $s=mysqli_query($conn, $command)or die(mysqli_error($command));
            if($s){
            ...
            echo "<script>alert('This theatre at this Location has already been registered')</script>";
        }
    }
    else{
